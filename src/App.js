    ...
    const filterhandler = async () => {
      ...
      try {
        let data = await fetch(
          `https://blackcoffer-assinment-api.vercel.app/api/?added=${addYearfilter}&sector=${sectorfilter}&end_year=${endYearFilter}&topic=${topicFilter}&region=${regionFilter}&start_year=${startYearFilter}&published=${publishedYearFilter}&country=${countryFilter}&pestle=${pestleFilter}&source=${sourceFilter}`
        ).then((res) => res.json());
        // filter field data
    ...
    <>
      <div className="App">
        <header className="App-header">
          <h1>Blackcoffer DashBoard</h1>
          <div className="filter">
            <SelectFild
              value={addYearfilter}
              setValue={setAddYearfilter}
              data={addedData}
              title={"Added Year filter"}
            />
            <SelectFild
              value={sectorfilter}
              setValue={setSectorfilter}
              data={sectorData}
              title={"Sector Filter"}
            />
            <SelectFild
              value={endYearFilter}
              setValue={setEndYearFilter}
              data={endYearData}
              title={"End Year Filter"}
            />
            <SelectFild
              value={topicFilter}
            ...
            <SelectFild
              ...
              setValue={setStartYearFilter}
              data={start_yearData}
              title={"Start Year Filter"}
            />
            <SelectFild
              value={publishedYearFilter}
              setValue={setPublishedYearFilter}
              data={publishedData}
              title={"Published Year Filter"}
            />
            <SelectFild
              value={countryFilter}
          ...
          <div className="filter">
            <Barplot
              width={300}
              height={250}
              data={intensitySector}
              text={"Top 5 Intensity Sector"}
            />
            <Barplot
              width={300}
              height={250}
              data={likelihoodSector}
              text={"Top 5 Likelihood Sector"}
            />
            <Barplot
              width={300}
              height={250}
              data={topicRepited}
              text={"Top 5 Repeated Topic"}
            />
            <Barplot
              width={300}
              height={250}
              data={countryContribute}
              text={"Top 5 Country Contribution"}
            />
            <PieChart
              width={700}
              height={400}
              data={regionContribute}
              text={"Top 5 Region Contribution"}
            />
          </div>
        </header>
      </div>
